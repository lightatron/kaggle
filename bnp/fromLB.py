# -*- coding: utf-8 -*-
"""
Created on Wed Mar  2 15:25:11 2016
...
extc = ExtraTreesClassifier(n_estimators=4000, max_features=0.3, criterion='entropy', min_samples_split=3,
                            max_depth=100, min_samples_leaf=2, n_jobs=3)

extc.fit(X_train,target)
y_pred = extc.predict_proba(X_test)
pd.DataFrame({"ID": id_test, "PredictedProb": y_pred[:,1]}).to_csv('extra_trees2.csv',index=False)     

print "Done for non-calib"        
#clf = CalibratedClassifierCV(extc, cv="prefit")
clf = CalibratedClassifierCV(extc, cv=5)
clf.fit(X_train, target)

print('Predict...')
y_pred = clf.predict_proba(X_test)
print "log loss: "  + str(metrics.log_loss(target, y_pred))
